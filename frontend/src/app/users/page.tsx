...
export default function UsersPage() {
  ...
  const { data: rankings, isLoading, isError } = useIndividualRank(franchise, subgroup, user);
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in duration-500">
      <div className="flex flex-row items-center justify-between gap-6 mb-8 border-b border-zinc-900 pb-6">
        ...
        <div className="relative">
          <select
            ...
            className={cn("appearance-none bg-surface border border-border px-4 py-2 pr-10 text-[10px] font-black uppercase tracking-widest text-white outline-none cursor-pointer hover:border-zinc-700 transition-colors", theme.focus)}
          >
            {user === "" && <option key="temp" value="">Select a User</option>}
      ...
      {isLoading && (
        <div className="space-y-2 animate-pulse">
           {[...Array(5)].map((_, i) => (
             <div key={i} className="h-12 bg-zinc-900/50 rounded w-full" />
           ))}
        </div>
      )}
      ...
      {isError && (
        <div className="p-8 border border-red-900/50 bg-red-900/10 rounded-lg text-center">
          <p className="text-red-400 font-bold uppercase tracking-widest text-xs">Failed to load rankings</p>
          <p className="text-zinc-500 text-sm mt-2">The user may not have any valid rankings for this subgroup.</p>
        </div>
      )}

      {!isLoading && !isError && sortedRankings && (
        <>
          <div className="flex flex-row items-center justify-end gap-5 mb-8 animate-in slide-in-from-top-2 duration-500">
          ...
          <div className="space-y-1 animate-in slide-in-from-bottom-4 duration-500 fade-in fill-mode-backwards">
      ...
      {!isLoading && !isError && !user && (
         <div className="text-center py-20 opacity-50">
            <PersonStandingIcon className="w-12 h-12 mx-auto mb-4 text-zinc-700" />
            <p className="uppercase tracking-widest text-xs font-bold text-zinc-600">Select a user to view rankings</p>
         </div>
      )}
    </div>
  )
